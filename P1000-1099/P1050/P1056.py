...
def best_cut():
    M, N, K, L, D = map(int, input().split())   # 读取输入, split是字符串分割函数
    ...
    # 从高到低排序，并且返回前K个和前L个的索引
    best_rows = sorted(range(M - 1), key = lambda x : row_edges[x], reverse=True)[:K]
    best_cols = sorted(range(N - 1), key = lambda x : col_edges[x], reverse=True)[:L]

    return sorted(best_rows), sorted(best_cols) # 再排序输出

if __name__ == "__main__":
    best_rows, best_cols = best_cut()
    print(" ".join(str(r + 1) for r in best_rows)) 
    print(" ".join(str(c + 1) for c in best_cols))
