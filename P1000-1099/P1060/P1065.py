...
if __name__ == "__main__":
    ...
    # 每台机器的已占用时间段列表：每个元素为 [start, end]
    machine_slot = [[] for _ in range(m)]
    ...
    for j in order:
        ...
        # ---- 在该机器的空档中寻找可放置时间段 ----
        slots = machine_slot[mach]
        start_time = earliest

        # 尝试插入空档
        placed = False
        for i in range(len(slots)):
            prev_end = slots[i - 1][1] if i > 0 else 0
            cur_start = slots[i][0]

            # 空档区间 = [prev_end, cur_start)
            begin = max(prev_end, earliest)
            if begin + dur <= cur_start:
                start_time = begin
                placed = True
                break
        ...
        if not placed:
            if slots:
                start_time = max(earliest, slots[-1][1])
            else:
                start_time = earliest
    ...
    # 最终完工时间 = 所有机器 end 的最大值
    ans = 0
    for s in machine_slot:
        if s:
            ans = max(ans, s[-1][1])
